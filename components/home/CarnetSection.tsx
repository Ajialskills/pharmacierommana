...
export default async function CarnetSection({ articles }: CarnetSectionProps) {
  ...
  return (
    <section aria-labelledby="carnet-heading" className="pt-24 pb-10 bg-[var(--color-surface-container-low)]">
      <div style={{ maxWidth: "var(--spacing-max-width)" }} className="mx-auto px-[var(--spacing-lg)]">
        ...
        <div className="grid lg:grid-cols-5 gap-[var(--spacing-gutter)]">
          ...
          <Link
            ...
            className="lg:col-span-3 group relative overflow-hidden rounded-2xl bg-white border border-[var(--color-border-subtle)] hover:shadow-xl transition-shadow flex flex-col"
          >
            {featured.cover_image ? (
              ...
              <div className="flex-1 min-h-[180px] bg-[color-mix(in_srgb,var(--color-primary)_8%,transparent)]" />
            )}
        ...
        {/* Third row — 3 more cards */}
        {rest.length > 5 && (
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-[var(--spacing-gutter)] mt-[var(--spacing-gutter)]">
            {rest.slice(5).map((article) => (
              <Link
                key={article.id}
                href={`/blog/${article.slug}`}
                className="group bg-white border border-[var(--color-border-subtle)] rounded-2xl overflow-hidden hover:shadow-md transition-shadow"
              >
                {article.cover_image ? (
                ...
                <div className="p-5">
                  ...
                  <h3 className="text-sm font-bold text-[var(--color-on-surface)] leading-snug line-clamp-2 group-hover:text-[var(--color-primary)] transition-colors mb-2">
                    {article.title}
                  </h3>
                  {article.excerpt && (
